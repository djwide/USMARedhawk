...
/***********************************************************************************************
    ...
    Messages:
        ...
        void PowerSensor_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }

        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &PowerSensor_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &PowerSensor_i::<callback method>)
        in the constructor.
        ...
        //Add to PowerSensor.cpp
        PowerSensor_i::PowerSensor_i(const char *uuid, const char *label) :
            PowerSensor_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &PowerSensor_i::scaleChanged);
            addPropertyListener(status, this, &PowerSensor_i::statusChanged);
        }

        void PowerSensor_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(PowerSensor_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }

        void PowerSensor_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(PowerSensor_i, "status changed");
        }

        //Add to PowerSensor.h
...
{
    LOG_DEBUG(PowerSensor_i, "serviceFunction() example log message");
