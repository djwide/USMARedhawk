...
void accumulator_i::constructor()
{
    ...
    ***********************************************************************************/
	messageIn->registerMessage("runningAve", this, &accumulator_i::messageReceived);

    LOG_DEBUG(accumulator_i, "1");
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void accumulator_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }

        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &accumulator_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &accumulator_i::<callback method>)
        in the constructor.
        ...
        //Add to accumulator.cpp
        accumulator_i::accumulator_i(const char *uuid, const char *label) :
            accumulator_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &accumulator_i::scaleChanged);
            addPropertyListener(status, this, &accumulator_i::statusChanged);
        }

        void accumulator_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(accumulator_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }

        void accumulator_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(accumulator_i, "status changed");
        }

        //Add to accumulator.h
...
************************************************************************************************/
void accumulator_i::messageReceived(const std::string &id, const runningAve_struct &msg){//this lines are changed

    LOG_DEBUG(accumulator_i, "2");
}
int accumulator_i::serviceFunction()
{
    LOG_DEBUG(accumulator_i, messageIn);
    return NOOP;
}
